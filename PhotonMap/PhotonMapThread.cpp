...
std::vector<Photon> PhotonMap::ShootPhotonsAsync(const int photonNum, const unsigned int seed)
{

  Real tMax=999999.9, tMin=0.00001;
  UniformRandomGenerator01 random(seed);

  std::vector<Photon> photonPool;
  ...
  for(int i=0; i<photonNum; i++){
    ...
    if(i%1000 == 0){std::cout << seed << ":" << i << "/" << photonNum << std::endl;}
    ///////////////////
    // get hit information
    ///////////////////
	...
	photonPool.push_back( Photon(hitPos, ray.Direction, flux) );
      }
...
void PhotonMap::ShootPhotons(const int photonNum)
{
  ...
  const int tnum =24;
  std::vector< std::future< std::vector<Photon> > > results(tnum);

  for(int i=0; i<tnum; i++){
    auto func =std::bind( &PhotonMap::ShootPhotonsAsync, this, photonNum/tnum, i+111);
    results[i] =std::async( std::launch::async, func);
  }
  

  for(int i=0; i<tnum; i++){
    std::vector<Photon> photonPool =results[i].get();
    for(int j=0; j<photonPool.size(); j++){
      this->Photons.AddElement( photonPool[j] );
    }
  }
